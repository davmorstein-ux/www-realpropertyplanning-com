      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
        {cards.map(({ Icon, label, description, cta, href }) => (
          <Link
            ...
            to={href}
            aria-label={label}
            className="group flex h-full flex-col items-center text-center rounded-2xl bg-white border-2 border-navy/15 p-8 lg:p-10 shadow-[0_12px_32px_-18px_hsl(var(--navy)/0.45)] hover:border-navy hover:shadow-[0_22px_46px_-22px_hsl(var(--navy)/0.65)] hover:-translate-y-1 transition-all duration-300 no-underline"
            style={{ textDecoration: "none" }}
          >
            <Icon className="w-12 h-12 text-navy mb-5" strokeWidth={1.75} aria-hidden="true" />
            <h3 className="font-serif text-2xl lg:text-3xl text-navy font-bold mb-3 leading-tight">
              {label}
            </h3>
            <p className="text-[#374151] mb-7 flex-1" style={{ fontSize: "17px", lineHeight: 1.6 }}>
              {description}
            </p>
            <span className="mt-auto inline-flex items-center justify-center gap-2 bg-navy group-hover:bg-[hsl(var(--navy))/0.9] text-white font-bold text-base px-6 py-3 rounded-lg transition-colors no-underline">
              {cta}
