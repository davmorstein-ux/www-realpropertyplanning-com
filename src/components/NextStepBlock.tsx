  ...
  <section className={`py-14 lg:py-20 ${background}`}>
    <div className="container px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <h2 className="font-serif text-2xl text-foreground font-semibold mb-6">{heading}</h2>
        <div className="space-y-4">
          {steps.map((step) => (
            <Link
              key={step.href}
              to={step.href}
              className="card-3d group flex items-center justify-between gap-4 p-5 no-underline"
            >
              <div>
              ...
              <img
                src={iconArrowRight3d}
                alt=""
                aria-hidden="true"
                className="w-6 h-6 object-contain shrink-0 opacity-40 group-hover:opacity-100 transition-opacity"
                loading="lazy"
                sizes="100vw"
                decoding="async"
                width={512}
