...
const faqs = [
  { question: "What should I do first with an inherited home in Kent?", answer: `Secure the property, confirm your legal authority, and get an honest assessment of its condition and value. ${FEATURED_BROKER.Role} can visit the home, evaluate what affects its marketability, and outline your options clearly before you make any decisions.` },
  { question: "How does an appraisal background help in Kent?", answer: `As both a licensed Washington State real estate broker and a state-certified residential appraiser, ${FEATURED_BROKER.role} provides pricing based on actual property condition and comparable sales — not automated estimates. In Kent, where home ages and conditions vary block by block, this dual perspective is especially valuable.` },
  { question: "What if siblings disagree about selling?", answer: `This happens often. ${FEATURED_BROKER.Role} provides objective, condition-based information — realistic value, preparation costs, and available options — so every family member works from the same facts. Clear data usually helps conversations move forward.` },
  { question: "Should I fix up a Kent inherited home before selling?", answer: `It depends on the property. ${FEATURED_BROKER.Role} evaluates which improvements are likely to pay for themselves and which aren't worth the cost or delay. Many Kent inherited homes sell well with targeted preparation rather than major renovation.` },
  { question: "Can you get help if I live outside Kent?", answer: `Yes. ${FEATURED_BROKER.Role} works with families managing properties from across Washington, out of state, and overseas. ${FEATURED_BROKER.pronoun.Subject} coordinates property access, vendor management, and the sale process so you don't need to make repeated trips.` },
  { question: "How long does it take to sell an inherited home in Kent?", answer: `Timelines vary based on the legal process, condition, and preparation. Once the property is ready and authority is established, well-priced Kent homes typically attract interest within a few weeks. ${FEATURED_BROKER.Role} helps set realistic expectations from the beginning.` },
];

const KentProbateEstate = () => {
  useEffect(() => {
    const faqSchema = { "@context": "https://schema.org", "@type": "FAQPage", mainEntity: faqs.map((f) => ({ "@type": "Question", name: f.question, acceptedAnswer: { "@type": "Answer", text: f.answer } })) };
    const localSchema = { ...hubOrganizationSchema, "@context": "https://schema.org", areaServed: [{ "@type": "City", name: "Kent", containedInPlace: { "@type": "State", name: "Washington" } }, ...areaServed] };
      ...
      <section className="py-20 lg:py-28 bg-background">
        <div className="container px-6 lg:px-8">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-muted-foreground text-lg leading-relaxed">As both a licensed Washington State real estate broker and a state-certified residential appraiser, {FEATURED_BROKER.role} provides that honest, condition-based assessment. The result is pricing you can trust and preparation advice that makes practical sense for your situation.</p>
          </div>
        </div>
      </section>

      <MidPageCTA heading="Questions About a Kent Property?" body={`${FEATURED_BROKER.name}, a licensed real estate broker, offers confidential property consultations for executors, trustees, and families handling inherited or estate property in Kent and South King County.`} buttonText="Schedule a Consultation" microcopy="No pressure. Just practical guidance on the property questions." />
      ...
      <section className="py-20 lg:py-28 bg-background">
        <div className="container px-6 lg:px-8">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-muted-foreground text-lg leading-relaxed mb-5">We provide a neutral, fact-based foundation. We assess the property, presents realistic value and preparation options, and gives every family member the same clear picture to work from. That shared understanding is usually what unlocks forward progress.</p>
            <p className="text-muted-foreground text-lg leading-relaxed">From there, we handle the execution — coordinating cleanout, managing contractors, and guiding the sale process — so the family can step back from the day-to-day logistics.</p>
          </div>
        </div>
      </section>
        ...
        <div className="container px-6 lg:px-8">
          <div className="max-w-3xl mx-auto">
            ...
            <ol className="space-y-6">
              {[{ title: "Listening to Your Situation", desc: `${FEATURED_BROKER.Role} starts by understanding the property, the people involved, and what's driving the timeline. Every family is different.` }, { title: "Evaluating the Property", desc: "I visit the home, assess its condition honestly, and identify what matters for pricing and marketability in Kent's specific market." }, { title: "Pricing It Right", desc: "Using my appraisal background and local comparable data, I develop pricing that reflects reality — not hopes or algorithms." }, { title: "Managing the Preparation", desc: "I coordinate cleanout, targeted improvements, and vendor logistics so the property is ready without wasted effort or money." }, { title: "Handling the Sale", desc: "From listing through closing, I manage the process with clear communication and no surprises. Straightforward from start to finish." }].map((step, i) => (
