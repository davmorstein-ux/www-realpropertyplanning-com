  ...
  <CountyPageTemplate
    ...
    aeoSupportFaqs={[
      ...
      },
      {
        ...
        answer: `Coastal exposure creates specific condition issues — moisture intrusion, weathering, salt air corrosion, and deferred maintenance on decks, roofing, and exterior finishes — that affect both value and buyer expectations. ${FEATURED_BROKER.name}'s condition-based valuation approach accounts for these factors specifically, ensuring estate properties are priced based on their actual condition rather than coastal location alone.`,
      },
    ]}
    ...
    communitiesIntro={`${FEATURED_BROKER.name} serves families, executors, and professionals throughout Pacific County, including:`}
    closingCtaBody={[
      ...
      `${FEATURED_BROKER.name} provides the condition-based, locally informed guidance that Pacific County's coastal and rural property landscape requires.`,
    ]}
    countySpecificFaqs={[
      {
        question: "Does Real Property Planning handle Pacific County probate listings before Letters are issued?",
        answer: `No — the property cannot be marketed until Letters Testamentary or Letters of Administration have been issued by the Pacific County Superior Court in South Bend. ${FEATURED_BROKER.name} uses that waiting period to conduct the date-of-death valuation walk-through, secure the property, coordinate cleanout vendors, and prepare the marketing file — so you're ready to move immediately when Letters arrive.`,
      },
      {
        question: "How is a Long Beach Peninsula vacation property valued as part of an estate?",
        answer: `Long Beach Peninsula vacation properties are valued based on location within the peninsula, proximity to the ocean or bay, lot size, condition, and rental history if applicable. Automated estimates are frequently unreliable for these properties. ${FEATURED_BROKER.name}'s certified appraisal background provides the hands-on, property-specific evaluation that coastal vacation estate properties require.`,
      },
      {
        question: "Can Real Property Planning manage a Pacific County estate sale for families in Seattle or out of state?",
        answer: `Yes — and this is the most common situation ${FEATURED_BROKER.name} handles in Pacific County, where many properties are owned by families based hours away. ${FEATURED_BROKER.pronoun.Subject} handles property assessment, vendor coordination, preparation, showings, and closing locally — with regular updates so families don't need to make repeated trips to the south coast.`,
      },
      {
        question: "What should executors know about selling property in Raymond or South Bend?",
        answer: `Raymond and South Bend have a more limited buyer pool than the Long Beach Peninsula, and estate properties here need to be priced realistically for local demand rather than coastal lifestyle premiums. Honest condition assessment and practical preparation are especially important in these communities, where overpricing leads to extended market time and eventual price reductions. ${FEATURED_BROKER.name} provides the straightforward guidance that serves these markets best.`,
      },
    ]}
  />
);
