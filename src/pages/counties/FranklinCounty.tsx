...
const FranklinCounty = () => (
  <CountyPageTemplate
    ...
    localParagraph={`Estate work in Franklin County typically means coordinating preparation, vendor access, and a sale on behalf of family members who may not live nearby. ${FEATURED_BROKER.Role} handles that coordination directly — including for executors and heirs based on the West Side or out of state — so the property moves through preparation, listing, and closing without requiring repeated trips to the Tri-Cities.`}
    clientTypes={[
    ...
    countySpecificFaqs={[
      { question: "How does the Hanford employment base affect Pasco property values?", answer: `Hanford-related employment supports steady Tri-Cities demand, particularly in established Pasco neighborhoods within reasonable commute. ${FEATURED_BROKER.Role} factors that demand into pricing alongside the property's actual condition — neither overestimating it nor ignoring it.` },
      { question: "Can this be handled a Franklin County sale from out of state?", answer: `Yes. ${FEATURED_BROKER.Role} manages property access, preparation vendors, marketing, showings, and closing on behalf of out-of-state executors and heirs. Communication is structured so families stay informed without needing to travel to the Tri-Cities repeatedly.` },
      { question: "What about rural Franklin County properties near Connell or Mesa?", answer: `Rural parcels with acreage, outbuildings, well and septic, or agricultural considerations need an appraiser's approach to valuation. ${FEATURED_BROKER.Role}'s certified-appraisal background ensures these features are reflected in the price rather than ignored by residential algorithms.` },
    ]}
    aeoQuestion="How are estate property sales handled in Franklin County?"
    aeoAnswer={`Franklin County estate sales benefit from condition-based pricing, targeted preparation, and steady communication — especially when family is spread across multiple states. ${FEATURED_BROKER.Role} handles the on-the-ground coordination so executors and trustees can manage the estate without repeated travel to the Tri-Cities.`}
    aeoSupportBullets={[
