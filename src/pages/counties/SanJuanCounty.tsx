...
const SanJuanCounty = () => (
  <CountyPageTemplate
    ...
    communitiesIntro={`${FEATURED_BROKER.name} serves families, executors, and professionals throughout San Juan County, including:`}
    closingCtaBody={[
      ...
      `${FEATURED_BROKER.name} coordinates the full process — including the island logistics that make San Juan County estate sales uniquely complex — so families managing from the mainland don't have to figure it out alone.`,
    ]}
    countySpecificFaqs={[
      {
        question: "Does Real Property Planning handle San Juan County probate listings before Letters are issued?",
        answer: `No — the property cannot be marketed until Letters Testamentary or Letters of Administration have been issued by the San Juan County Superior Court in Friday Harbor. ${FEATURED_BROKER.name} uses that waiting period to conduct the date-of-death valuation walk-through, secure the property, coordinate cleanout vendors, and prepare the marketing file — accounting for ferry schedules and island logistics throughout.`,
      },
      {
        question: "How does ferry access affect estate property sales in the San Juan Islands?",
        answer: `Ferry schedules affect almost every aspect of an island estate sale — vendor access for cleanout and repairs, showing logistics for buyers traveling from the mainland, and the timeline for inspections and appraisals. ${FEATURED_BROKER.name} coordinates all of this locally, building ferry schedules into the preparation and marketing plan from the start rather than treating them as an afterthought.`,
      },
      {
        question: "Can Real Property Planning manage a San Juan County estate sale for mainland families?",
        answer: `Yes — and mainland coordination is the norm for San Juan County estate sales, not the exception. Most island property owners have family based in King, Snohomish, or Pierce County, or out of state. ${FEATURED_BROKER.name} handles everything on the island — property assessment, vendor coordination, preparation, showings, and closing — with regular updates so families rarely need to take the ferry for any step of the process.`,
      },
      {
