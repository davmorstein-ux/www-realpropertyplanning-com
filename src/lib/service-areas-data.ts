...
const cityContent: Record<string, Omit<CityData, "name" | "slug" | "county" | "countySlug">> = {
  ...
  bellevue: {
    ...
    aeoAnswer: `Bellevue's market rewards accurate pricing and professional presentation — even for estate properties with deferred maintenance. Buyers in Bellevue have high expectations for condition and finish, which means the preparation strategy matters as much as the pricing. ${FEATURED_BROKER.name} evaluates each property with both a broker's understanding of buyer expectations and an appraiser's methodology for condition-adjusted value — so families get honest guidance on what to address before listing and what to leave alone.`,
    localFaqs: [
      { question: "How is selling an inherited Bellevue home different from a standard sale?", answer: `Bellevue's buyer pool includes tech-sector employees, international buyers, and move-up families — all of whom have specific expectations. An estate home with dated finishes or deferred maintenance needs to be priced and positioned carefully to appeal to the buyers most likely to see its value. ${FEATURED_BROKER.name}'s preparation analysis helps families make targeted decisions rather than over-investing in renovations that don't deliver return.` },
      { question: "What about inherited condos in the Bellevue downtown corridor?", answer: `Condo sales in Bellevue involve HOA documentation, building financials, and resale certificate requirements that add complexity to an already detailed process. ${FEATURED_BROKER.name} coordinates the full documentation process so executors don't have to manage it independently.` },
    ],
  },
  kirkland: {
    tone: "suburban",
    ...
    seoDescription: "Estate property sales, inherited home guidance, and senior transitions in Kirkland, Washington. Local expertise from Real Property Planning, based in Kirkland.",
    heroHeading: "Probate & Estate Real Estate in Kirkland, Washington",
    heroIntro: `Kirkland is home to Real Property Planning — which means ${FEATURED_BROKER.name}'s knowledge of this market goes beyond professional research. From the waterfront neighborhoods near Lake Washington to the established communities of North Kirkland and Totem Lake, ${FEATURED_BROKER.firstName} has worked with properties across the full range of Kirkland's housing landscape. For families navigating a probate sale, senior transition, or inherited property in Kirkland, that local depth translates directly into better pricing, more targeted preparation, and a smoother process.`,
    ...
    brokerAppraiserValue: `Kirkland's appeal lies in its location, walkability, and neighborhood character — but those qualities can be difficult to quantify with automated tools. ${FEATURED_BROKER.name} evaluates each property based on its actual condition, lot characteristics, and realistic buyer demand in Kirkland. ${FEATURED_BROKER.name}'s appraisal background provides an analytical foundation that supports confident pricing decisions, especially for properties that need preparation or have deferred maintenance.`,
    localSituations: [
  ...
  redmond: {
    ...
    heroIntro: `Redmond's real estate market reflects its dual identity — a technology hub anchored by Microsoft and Nintendo alongside established residential neighborhoods that predate the tech boom by decades. Estate properties in Redmond range from mid-century homes in older neighborhoods to newer construction in master-planned communities, each with its own buyer expectations and pricing dynamics. ${FEATURED_BROKER.name} works with executors and families throughout Redmond, bringing the local Eastside expertise needed to navigate this varied market accurately.`,
    localKnowledge: `Redmond's housing stock includes everything from older homes in downtown Redmond and Education Hill to newer construction in Redmond Ridge, Trilogy, and the Overlake area. These submarkets behave differently, and an estate or inherited property may need very different preparation and pricing strategies depending on location. ${FEATURED_BROKER.name}'s familiarity with Redmond's micro-markets helps clients avoid the common mistake of applying a one-size-fits-all approach.`,
    brokerAppraiserValue: `Redmond's tech-driven market creates wide value spreads between neighborhoods — an Education Hill rambler, an Overlake townhome near transit, and a Trilogy active-adult property each serve fundamentally different buyer pools. ${FEATURED_BROKER.name}'s certified appraisal credentials allow ${FEATURED_BROKER.pronoun.object} to evaluate which comparables truly apply based on a property's specific location, age, and condition, providing the defensible analysis that Redmond estate and transition situations require.`,
    localSituations: [
  ...
  bothell: {
    ...
    heroIntro: "Bothell straddles the King and Snohomish County line — which means estate properties here may fall under different county probate processes depending on which side of the border they sit on. Real Property Planning is familiar with both county systems and serves the full Bothell area, including Canyon Park, the downtown core, and the surrounding communities of Kenmore and Woodinville. For executors managing an estate property in Bothell, this cross-county expertise matters.",
    localKnowledge: `Bothell's real estate spans mid-century homes in the original downtown core, newer construction near Canyon Park and the University of Washington Bothell campus, and properties along the river corridor. The city's position across two counties means comparable sales may come from either side of the boundary, and buyer demographics can shift meaningfully between neighborhoods. ${FEATURED_BROKER.name}'s experience with Bothell's specific market dynamics helps clients price and position properties accurately.`,
    brokerAppraiserValue: `Bothell's dual-county position creates a valuation challenge that most brokers don't encounter — comparable sales from King County and Snohomish County may support different pricing conclusions for the same property. ${FEATURED_BROKER.name}'s certified appraisal credentials allow ${FEATURED_BROKER.pronoun.object} to evaluate which comparables truly apply based on the property's specific location, buyer pool, and neighborhood context. This cross-county analytical foundation is especially valuable for estate and transition properties where defensible pricing is essential.`,
    localSituations: [
    ...
    aeoQuestion: "What should executors know about estate sales in the Bothell area?",
    aeoAnswer: `Bothell's market has strengthened significantly in recent years, making it attractive for estate property sales — provided the property is priced and prepared correctly. The cross-county nature of Bothell means executors should confirm which county the property sits in, as probate procedures differ between King and Snohomish Counties. ${FEATURED_BROKER.name} can help clarify this early and coordinate with the appropriate legal and title professionals.`,
  },
  issaquah: {
    tone: "suburban",
    ...
    aeoAnswer: `Issaquah's segmented market — from Highlands planned communities to Olde Town homes — means inherited properties compete in very different buyer pools depending on location. ${FEATURED_BROKER.name} evaluates each property's specific context and provides practical, evidence-based guidance on preparation, pricing, and timing.`,
    aeoSupportBullets: [
    ...
    localFaqs: [
      { question: "What are the first steps when selling an inherited home in Issaquah?", answer: `Secure the property, confirm your legal authority to act, and get a realistic assessment of condition and value. ${FEATURED_BROKER.name} can visit the property, evaluate what needs attention, and help you develop a clear plan — whether the home is in Olde Town, the Highlands, Talus, or the surrounding area.` },
      { question: `How does ${FEATURED_BROKER.firstName} determine the right price for an Issaquah estate property?`, answer: `Through a condition-adjusted comparable analysis grounded in certified appraisal methodology. This means ${FEATURED_BROKER.firstName} doesn't just pull nearby sales — ${FEATURED_BROKER.firstName} evaluates how the subject property's specific condition, location within Issaquah, and current buyer demand affect its realistic market position. The result is a defensible price that holds up to scrutiny from co-heirs, attorneys, and the market itself.` },
      { question: `Does ${FEATURED_BROKER.firstName} help with cleanout and preparation?`, answer: `Yes — ${FEATURED_BROKER.firstName} coordinates with trusted local vendors for estate cleanout, targeted repairs, and staging as needed. ${FEATURED_BROKER.pronoun.Subject} evaluates each potential improvement through a return-on-investment lens so families don't over-spend on preparation that won't meaningfully improve the outcome.` },
    ],
  },
  sammamish: {
    tone: "premium",
  ...
  "mercer-island": {
    ...
    aeoAnswer: `Mercer Island's premium market demands valuation expertise that reflects the island's unique buyer pool and property characteristics. ${FEATURED_BROKER.name}'s dual credentials as a broker and certified appraiser provide the defensible, condition-based pricing that executors, trustees, and families need when significant value is at stake.`,
    aeoSupportFaqs: [
      ...
      { "question": "Can a Mercer Island estate sale be managed remotely?", "answer": `Yes. ${FEATURED_BROKER.name} handles property assessment, preparation coordination, marketing, and closing — keeping out-of-state families informed throughout.` },
    ],
  },
  newcastle: {
    tone: "premium",
    ...
    heroIntro: `Newcastle sits between Bellevue and Renton — benefiting from Eastside proximity while maintaining its own community character. Estate properties here often include larger lots and established landscaping that require specific preparation and pricing considerations. ${FEATURED_BROKER.name} works with executors and families in Newcastle with the same evidence-based approach ${FEATURED_BROKER.pronoun.subject} brings to every community in King County.`,
    localKnowledge: `Newcastle's housing stock is predominantly newer, with many homes built in the 1990s and 2000s in planned communities like Newcastle Highlands and the Golf Club at Newcastle area. Views are a primary value driver here, and the difference between a property with a panoramic view and one without can be substantial. ${FEATURED_BROKER.name} helps clients understand how view corridors, lot positioning, and condition affect market value in Newcastle's hillside environment.`,
  },
  seattle: {
    tone: "urban",
    ...
    aeoSupportBullets: [
      ...
      "Full coordination for executors managing the process from out of state",
      "Established relationships with local cleanout crews, contractors, and senior move professionals",
    ],
    localFaqs: [
      { question: "What are the first steps when selling an inherited home in Seattle?", answer: `Secure the property, confirm your legal authority to act, and get an honest condition and value assessment before making any decisions. ${FEATURED_BROKER.name} can visit the property, walk through what needs attention, and help you develop a clear, realistic plan — whether the home is in Magnolia or Rainier Valley, Capitol Hill or West Seattle.` },
      ...
      { question: `Does ${FEATURED_BROKER.firstName} handle estate properties in all Seattle neighborhoods?`, answer: `Yes — ${FEATURED_BROKER.firstName} serves the full city of Seattle, including Capitol Hill, Ballard, Magnolia, West Seattle, Beacon Hill, Rainier Valley, Northgate, Shoreline-adjacent neighborhoods, and everything in between. Neighborhood knowledge is a core part of how ${FEATURED_BROKER.firstName} approaches pricing and preparation strategy.` },
    ],
  },
  renton: {
    tone: "urban",
  ...
  auburn: {
    ...
    localKnowledge: `Auburn's housing market includes mid-century homes in the valley, newer developments on the hillside areas, and properties with larger lots along the rural edges. The city's proximity to major employers and transportation corridors influences buyer demand in ways that vary by neighborhood. ${FEATURED_BROKER.name} helps Auburn clients understand these dynamics and position estate or transition properties effectively based on their specific location and condition.`,
  },
  "federal-way": {
    tone: "suburban",
  ...
  seatac: {
    ...
    localKnowledge: `SeaTac's housing market is compact and influenced by its airport-adjacent location. Properties vary significantly in value based on their proximity to flight paths, commercial zones, and the quieter residential pockets of the city. ${FEATURED_BROKER.name} helps clients navigate SeaTac's unique market dynamics with honest, condition-based guidance that accounts for the factors specific to this community.`,
  },
  tukwila: {
    tone: "urban",
    ...
    localKnowledge: `Tukwila's residential areas include neighborhoods near Foster Point, Riverton Heights, and the hillside areas with views toward the Cascades. The city's commercial centers, light rail access, and proximity to major employers all influence buyer demographics. ${FEATURED_BROKER.name} helps clients understand how Tukwila's unique position as both a commercial hub and residential community affects realistic property values.`,
  },
  woodinville: {
    tone: "rural",
    ...
    heroIntro: `Woodinville is home to Real Property Planning's mailing address — and it's a community ${FEATURED_BROKER.firstName} knows well. From the wine country properties along the Sammamish River corridor to established neighborhoods in Hollywood Hill and Bear Creek, Woodinville's real estate market rewards local knowledge. Estate properties here often include larger lots, horse properties, and rural-adjacent parcels that require specific expertise to price and position correctly. ${FEATURED_BROKER.name} works with families and executors throughout the Woodinville area with the local familiarity that only comes from being part of the community.`,
    localKnowledge: `Woodinville's housing market includes everything from newer townhomes and condos near the town center to larger-lot properties and homes with acreage in the surrounding areas. The Sammamish River corridor, wine district proximity, and equestrian properties all create niche market segments. ${FEATURED_BROKER.name}'s appraisal credentials are especially valuable here, where property characteristics like land area, outbuildings, and rural features can meaningfully affect value.`,
    ...
    aeoAnswer: `Woodinville's mix of established neighborhoods, wine country adjacency, and newer developments creates pricing nuances that require local expertise. ${FEATURED_BROKER.name} evaluates each property based on its specific location, condition, and buyer pool — providing the honest, practical, evidence-based guidance that families need during property transitions.`,
    aeoSupportBullets: [
  ...
  kenmore: {
    ...
    localKnowledge: `Kenmore's housing includes waterfront and water-view properties along Lake Washington, established mid-century homes in the hillside neighborhoods, and newer developments near the city center. Proximity to the lake, trail access, and views all significantly influence value. ${FEATURED_BROKER.name} helps Kenmore clients understand how these factors — combined with property condition — affect realistic market positioning.`,
  },
  "maple-valley": {
    tone: "suburban",
  ...
  duvall: {
    ...
    localKnowledge: `Duvall's housing market includes homes in the walkable town center, newer subdivisions, and larger rural properties along the valley. The community's small-town identity and agricultural surroundings create a market where buyer expectations differ from more urban areas. ${FEATURED_BROKER.name}'s appraisal background helps clients in Duvall understand how property size, rural features, and community character affect realistic valuation.`,
    brokerAppraiserValue: `Duvall's Snoqualmie Valley location means many properties have features — river proximity, acreage, agricultural outbuildings, equestrian facilities — that automated tools trained on suburban data consistently misjudge. ${FEATURED_BROKER.name}'s certified appraisal background provides the property-specific analysis Duvall clients need, especially when estate or trust situations require defensible pricing for homes that don't fit standard residential comparable patterns.`,
    localSituations: [
  ...
  edmonds: {
    ...
    aeoAnswer: `Edmonds' walkable downtown, waterfront character, and strong buyer demand create specific expectations for estate properties. ${FEATURED_BROKER.name}'s dual credentials as a broker and certified appraiser help families price inherited homes accurately — accounting for the condition gaps between estate properties and the updated homes Edmonds buyers typically expect.`,
    aeoSupportFaqs: [
      { "question": "How does waterfront proximity affect estate property values in Edmonds?", "answer": `Significantly — and the premium varies depending on whether the property has water views, is within walking distance of the waterfront, or simply benefits from the Edmonds zip code. ${FEATURED_BROKER.name}'s appraisal-grounded analysis accounts for these distinctions precisely, rather than applying a blanket waterfront premium that may not reflect the property's specific situation.` },
      { "question": "Can an Edmonds estate sale be coordinated remotely?", "answer": `Yes. ${FEATURED_BROKER.name} manages the full process locally — property assessment, preparation, listing, and closing — with regular updates for out-of-area families.` },
    ],
  },
  "mountlake-terrace": {
    tone: "suburban",
  ...
  brier: {
    ...
    localKnowledge: `Brier's housing stock tends toward older single-family homes on lots that are significantly larger than those in neighboring communities. The community's zoning protects its semi-rural character, which appeals to buyers seeking privacy and space. ${FEATURED_BROKER.name} helps clients understand how Brier's unique lot sizes, mature landscaping, and residential character affect property values — factors that automated estimates often misjudge in a community this small.`,
  },
  mukilteo: {
    tone: "waterfront",
  ...
  everett: {
    ...
    aeoAnswer: `Everett's evolving market offers opportunity for estate property sales — but realistic pricing and practical preparation are key. ${FEATURED_BROKER.name} evaluates each property's condition, identifies cost-effective improvements, and manages the full sale process for executors and families who need clear, honest guidance.`,
    aeoSupportBullets: [
    ...
    localFaqs: [
      { question: "What are the first steps when selling an inherited home in Everett?", answer: `Secure the property, confirm legal authority, and get an honest assessment of condition and realistic value — before making any decisions about repairs or listing. ${FEATURED_BROKER.name} can visit the property, evaluate what needs attention, and help develop a clear plan tailored to the specific neighborhood.` },
      { question: "How does Everett's market affect estate property pricing?", answer: `Everett's ongoing revitalization — particularly near the waterfront and downtown core — has created significant variation between neighborhoods. Properties in areas benefiting from development investment may perform better than broad city averages suggest; properties in more static neighborhoods need to be priced accordingly. ${FEATURED_BROKER.name}'s analysis accounts for these distinctions specifically.` },
    ],
  },
  "mill-creek": {
    tone: "suburban",
  ...
  "woodinville-snohomish": {
    ...
    localKnowledge: `Properties in the Snohomish County Woodinville area range from estate-style homes near the wine district to rural properties with acreage and newer suburban developments. ${FEATURED_BROKER.name}'s appraisal background helps clients in this area understand how the blend of rural, suburban, and wine-country characteristics affects property values.`,
  },
  snohomish: {
    tone: "smalltown",
    ...
    aeoAnswer: `Snohomish's small-town character and rural-adjacent properties create valuation challenges that automated tools handle poorly. ${FEATURED_BROKER.name}'s certified appraisal credentials allow ${FEATURED_BROKER.pronoun.object} to evaluate acreage, outbuildings, and non-standard features with the accuracy that estate and trust sales demand.`,
    aeoSupportFaqs: [
      ...
      { "question": "Can I get help with a Snohomish property I inherited from out of state?", "answer": `Yes. ${FEATURED_BROKER.name} coordinates the full process locally — assessment, cleanout, preparation, and sale — with regular updates so you can manage from a distance.` },
    ],
  },
  "lake-stevens": {
    tone: "suburban",
  ...
  arlington: {
    ...
    localKnowledge: `Arlington's housing includes established homes in the downtown area, suburban developments, and properties with acreage in the surrounding areas toward Darrington and the foothills. The community's balance of small-city accessibility and rural character creates a market where property features like land size, outbuildings, and road access meaningfully affect value. ${FEATURED_BROKER.name}'s appraisal credentials help Arlington clients understand these dynamics.`,
  },
  monroe: {
    tone: "smalltown",
    ...
    localKnowledge: `Monroe's housing ranges from homes near the historic downtown and fairgrounds to newer subdivisions and larger rural properties along the valley. The community's accessibility via US-2, its proximity to both employment centers and outdoor recreation, and its range of property sizes all influence buyer demographics. ${FEATURED_BROKER.name} helps Monroe clients understand how these factors affect value for estate and transition properties.`,
  },
  stanwood: {
    tone: "smalltown",
  ...
  "gold-bar": {
    ...
    localKnowledge: `Gold Bar's housing market is limited in inventory and includes a mix of older cabins, established homes, and newer construction, many with larger lots or river proximity. The community's rural character and distance from major employment centers shape the local buyer pool. ${FEATURED_BROKER.name}'s appraisal credentials help clients understand how Gold Bar's unique property types and small-market dynamics affect realistic pricing.`,
  },
  "granite-falls": {
    tone: "rural",
  ...
  tacoma: {
    ...
    aeoAnswer: `Tacoma's diverse neighborhoods — from the North End to Hilltop to the waterfront — create distinct buyer pools and pricing dynamics for estate properties. ${FEATURED_BROKER.name} provides practical, practical, evidence-based guidance that helps executors and families navigate preparation decisions, pricing strategy, and the full sale process with confidence.`,
    aeoSupportBullets: [
  ...
  "university-place": {
    ...
    heroIntro: `Lakewood and University Place are established Pierce County communities with distinct characters — Lakewood's diverse housing stock and military-adjacent economy, and University Place's waterfront-adjacent neighborhoods and strong community identity. Estate properties in both communities require pricing that reflects the specific neighborhood rather than broad county trends. ${FEATURED_BROKER.name} works with executors and families throughout both communities with the condition-adjusted, location-specific guidance these markets require.`,
  ...
  "gig-harbor": {
    ...
    aeoSupportFaqs: [
      ...
      { "question": "Can a Gig Harbor estate sale be coordinated from a distance?", "answer": `Yes. ${FEATURED_BROKER.name} manages property assessment, preparation, marketing, and closing — providing regular updates to families and fiduciaries who aren't local.` },
    ],
  },
  fircrest: {
    tone: "suburban",
  ...
  sumner: {
    ...
    localKnowledge: `Sumner's housing includes established homes near downtown, newer construction on the edges of the city, and properties influenced by the valley's agricultural context. The community's small-town character and proximity to Puyallup and Auburn create a buyer pool with specific expectations. ${FEATURED_BROKER.name} helps Sumner clients position estate and transition properties realistically within this local context.`,
  },
  orting: {
    tone: "smalltown",
  ...
  graham: {
    ...
    localKnowledge: `Graham's housing ranges from established homes on larger lots to newer subdivisions and properties with acreage, outbuildings, and agricultural uses. The community's rural character means property features like land area, well and septic systems, and road access meaningfully affect value. ${FEATURED_BROKER.name}'s appraisal credentials help Graham clients understand how these rural-specific factors influence realistic property pricing.`,
  },
  dupont: {
    tone: "military",
  ...
  fife: {
    ...
    localKnowledge: `Fife's residential areas are relatively compact, with established neighborhoods surrounded by commercial and industrial zones. The community's location along major transportation corridors and proximity to Tacoma and Federal Way influence buyer demographics. ${FEATURED_BROKER.name} helps Fife clients navigate the community's specific market dynamics with clear, condition-based guidance.`,
  },
  milton: {
    tone: "suburban",
    ...
    localKnowledge: `Milton's housing is primarily established single-family homes in a compact residential setting. The community's position across two counties and its proximity to Edgewood, Federal Way, and Fife influence comparable sales and buyer demographics. ${FEATURED_BROKER.name} helps Milton clients understand how these cross-boundary dynamics and the community's small-market character affect realistic property values.`,
  },
  edgewood: {
    tone: "rural",
    ...
    localKnowledge: `Edgewood's housing tends toward larger-lot single-family homes with a rural-residential feel, along with some newer development. The community's zoning protects its semi-rural character, which appeals to buyers seeking space and privacy. ${FEATURED_BROKER.name} helps clients understand how Edgewood's distinctive lot sizes and residential character affect property values — factors that generic pricing tools often undervalue.`,
  },
  buckley: {
    tone: "smalltown",
  ...
  eatonville: {
    ...
    localKnowledge: `Eatonville's housing ranges from established homes near the compact town center to rural properties with acreage and mountain access. The community's tourism connection to Mount Rainier and its agricultural character create a niche buyer pool. ${FEATURED_BROKER.name} helps Eatonville clients understand how the community's rural character and limited market activity affect realistic property values.`,
  },
  ...
  silverdale: {
    ...
    aeoAnswer: `Silverdale's proximity to Naval Base Kitsap and its blend of established and newer neighborhoods create specific dynamics for estate property sales. ${FEATURED_BROKER.name} provides practical guidance on pricing, preparation, and family coordination — helping executors and trustees navigate the process with clarity.`,
    aeoSupportBullets: [
  ...
  poulsbo: {
    ...
    localKnowledge: `Poulsbo's housing ranges from historic homes near the waterfront and downtown to newer developments and larger rural properties in the surrounding area. Waterfront access, Liberty Bay views, and proximity to downtown all significantly influence value. ${FEATURED_BROKER.name}'s appraisal credentials are particularly valuable in Poulsbo, where the range of property types and the community's distinctive character make generic pricing approaches unreliable.`,
    brokerAppraiserValue: `Poulsbo's mix of waterfront homes, village-adjacent properties, and rural-residential lots creates a market where automated pricing tools fall short. ${FEATURED_BROKER.name} evaluates each property on its actual characteristics — Liberty Bay proximity, view corridors, lot topography, and condition — providing the kind of grounded analysis that Poulsbo sellers and their advisors need for confident decisions.`,
    localSituations: [
    ...
    aeoAnswer: `Poulsbo's Scandinavian-influenced character, waterfront setting, and tight-knit community create a distinctive market for estate properties. ${FEATURED_BROKER.name}'s dual credentials as a broker and certified appraiser provide the valuation depth needed for accurate pricing — especially for properties with waterfront proximity or unique architectural character.`,
    aeoSupportBullets: [
  ...
  "bainbridge-island": {
    ...
    aeoSupportFaqs: [
      ...
      { "question": "Can the full sale process be managed on Bainbridge Island?", "answer": `Yes. ${FEATURED_BROKER.name} coordinates property assessment, preparation, marketing, and closing — handling ferry logistics and local vendor coordination for families who aren't on-island.` },
    ],
  },
  "port-orchard": {
    tone: "waterfront",
  ...
  "mount-vernon": {
    ...
    aeoQuestion: "How do estate and probate property sales work in Mount Vernon?",
    aeoAnswer: `Mount Vernon estate properties benefit from the city's position as Skagit County's commercial center, with steady local demand and spillover from buyers priced out of King and Snohomish Counties. Probate proceedings are administered through Skagit County Superior Court in Mount Vernon. ${FEATURED_BROKER.name} provides condition-adjusted analysis and full local coordination for families managing the process remotely.`,
    aeoSupportBullets: [
      "Locally coordinated property assessment and sale management",
      "Condition-adjusted pricing calibrated to the Mount Vernon market",
      "Full coordination for families managing from Seattle or out of state",
      "Familiarity with Skagit County probate court processes",
    ],
    localFaqs: [
      {
        question: "What are the first steps when selling an inherited home in Mount Vernon?",
        answer: `Secure the property, confirm your legal authority to act, and get an honest assessment of condition and value before making any decisions. For Skagit County estate properties, probate proceedings are administered through Skagit County Superior Court in Mount Vernon — your estate attorney can advise on the specific legal requirements for your situation. ${FEATURED_BROKER.name} can assess the property, help you understand your options, and coordinate the full sale process locally.`,
      },
      {
        question: "How active is the Mount Vernon real estate market for estate properties?",
        answer: `Mount Vernon's market reflects Skagit County's broader dynamics — steady demand from local buyers, some spillover from buyers priced out of King and Snohomish County markets, and specific interest from buyers seeking the agricultural and small-city character the area offers. Estate properties in good condition and priced accurately for their actual condition typically attract motivated buyers. ${FEATURED_BROKER.name}'s condition-adjusted analysis ensures the pricing reflects the Mount Vernon market specifically.`,
      },
    ],
  },
  "burlington": {
    ...
    localFaqs: [
      {
        ...
        answer: `These communities attract buyers who are specifically seeking what they offer — rural character, agricultural surroundings, small-community lifestyle. Marketing to that buyer means understanding them: where they come from, what they're looking for, and what they'll pay for the right property in the right condition. ${FEATURED_BROKER.name}'s approach to small Skagit community sales is calibrated to the actual buyer demand in these specific locations, not derived from broader county averages that don't apply here.`,
      },
      {
        question: "How does the farm-to-table and rural character of Bow and Edison affect property values?",
        answer: "It creates a specific premium for properties that embody that character — but only when they're priced and presented correctly. Properties that feel authentically connected to the community's identity attract motivated buyers willing to pay for what they can't find elsewhere. Properties that are overpriced or poorly presented lose that audience quickly in a market where the buyer pool is limited.",
      },
    ],
  },
};
...
export const services: ServiceData[] = [
  {
    ...
    cityIntroTemplate: `Selling a probate or estate property in {city} takes local market knowledge, realistic pricing, and experienced coordination. ${FEATURED_BROKER.name} guides executors, attorneys, and families through the sale of estate-owned real estate in {city} and {county}.`,
    targetAudience: [
    ...
    whyValuationMatters: `Estate properties often present in non-standard condition — deferred maintenance, accumulated belongings, outdated finishes. Automated valuations and generic market comparisons frequently miss the mark. ${FEATURED_BROKER.name}'s dual credentials as a licensed broker and certified appraiser allow ${FEATURED_BROKER.pronoun.object} to assess actual condition, identify realistic value ranges, and develop pricing strategies that protect estate interests while attracting qualified buyers.`,
  },
  {
    ...
    cityIntroTemplate: `Selling a longtime family home in {city} involves more than listing a property. ${FEATURED_BROKER.name} helps families plan and coordinate the transition — from preparation and pricing to vendor coordination and sale timing.`,
    targetAudience: [
  ...
  },
  {
    ...
    cityIntroTemplate: `Downsizing from a family home in {city} is a major decision. ${FEATURED_BROKER.name} helps homeowners evaluate their options, prepare the property, and manage the sale with clarity and realistic expectations.`,
    targetAudience: [
  ...
  },
  {
    ...
    cityIntroTemplate: `As an executor or personal representative managing property in {city}, you face real responsibility and often unfamiliar decisions. ${FEATURED_BROKER.name} provides experienced guidance for executors in {city} — from initial property assessment through sale and closing.`,
    targetAudience: [
  ...
  },
  {
    ...
    cityIntroTemplate: `Attorneys and fiduciaries managing property in {city} need a real estate partner who understands legal timelines, fiduciary obligations, and clear communication. ${FEATURED_BROKER.name} works with legal and fiduciary professionals across {city} and {county}.`,
    targetAudience: [
    ...
    whyValuationMatters: `Legal professionals need a real estate partner who can provide defensible pricing opinions, clear documentation, and reliable communication. ${FEATURED_BROKER.name}'s appraiser credentials and broker experience create a combination that supports legal and fiduciary decision-making with objective, market-grounded information.`,
  },
  {
    ...
    whyValuationMatters: `Many property decisions — from estate distribution to sale pricing — depend on an accurate understanding of value. Online estimates and automated tools often fail to account for condition, deferred maintenance, or unique property characteristics. ${FEATURED_BROKER.name}'s state-certified appraisal background provides a level of valuation insight that goes well beyond what a typical broker can offer.`,
  },
  {
    ...
    cityIntroTemplate: `Preparing a home for sale in {city} often involves more than cosmetic updates — especially for estate, inherited, or longtime family properties. ${FEATURED_BROKER.name} coordinates cleanout, repairs, and presentation strategy for properties in {city} so sellers can focus on the bigger picture.`,
    targetAudience: [
    ...
    whyValuationMatters: `Not every repair or update delivers a return. ${FEATURED_BROKER.name}'s appraisal background helps clients understand which improvements are likely to affect value and which are not — avoiding unnecessary spending while focusing preparation efforts where they matter most.`,
  },
  {
    ...
    cityIntroTemplate: `Selling a home during a divorce in {city} requires discretion, objectivity, and an understanding of how legal timelines affect the sale process. ${FEATURED_BROKER.name} provides experienced guidance for divorce-related property sales in {city} and throughout {county}.`,
    targetAudience: [
    ...
    whyValuationMatters: `Divorce-related sales often involve disagreements about value. ${FEATURED_BROKER.name}'s appraiser credentials provide an objective, defensible basis for pricing discussions — helping both parties and their attorneys move forward with confidence rather than speculation.`,
  },
  {
    ...
    whyValuationMatters: `Trust property sales carry fiduciary obligations. Trustees need defensible pricing, clear documentation, and a transparent process. ${FEATURED_BROKER.name}'s combined broker and appraiser credentials provide the kind of objective, well-supported guidance that meets fiduciary standards.`,
  },
];
...
export function getWhyLocalMatters(cityName: string, countyName: string, cityData?: CityData): string {
  ...
  return `Every neighborhood in ${cityName} has its own pricing dynamics, buyer expectations, and property characteristics. A evidence-based approach — one that accounts for condition, location, and realistic market positioning — helps clients avoid costly mispricing and unnecessary delays. ${FEATURED_BROKER.name}'s dual credentials as a licensed Washington real estate broker and state-certified residential appraiser provide a level of market insight that benefits clients throughout ${countyName}.`;
}
...
export function getToneHowWeHelpHeading(tone?: CommunityTone, serviceSlug?: string): string {
  ...
  switch (s) {
    case "probate-estate-sales":
      switch (t) {
        case "premium": return `How ${FEATURED_BROKER.firstName} Guides Estate Property Sales`;
        case "urban": return "Experienced Probate Sale Support";
        case "waterfront": return `How ${FEATURED_BROKER.firstName} Helps With Estate Property`;
        case "smalltown": return "Guiding Your Estate Sale, Step by Step";
        case "military": return "Practical Probate Sale Guidance";
        case "rural": return `How ${FEATURED_BROKER.firstName} Helps With Estate Property Sales`;
    ...
    case "senior-transitions":
      switch (t) {
        ...
        case "waterfront": return `How ${FEATURED_BROKER.firstName} Supports Senior Moves`;
        case "smalltown": return "Walking With You Through the Transition";
        case "military": return "Practical Senior Transition Guidance";
        case "rural": return `How ${FEATURED_BROKER.firstName} Helps Families in Transition`;
        default: return "How Real Property Planning Supports Senior Transitions";
      }
    case "downsizing-services":
      switch (t) {
        case "premium": return `How ${FEATURED_BROKER.firstName} Supports Your Downsizing Plan`;
        case "urban": return "Practical Downsizing Guidance";
        case "waterfront": return `How ${FEATURED_BROKER.firstName} Helps With Downsizing`;
        case "smalltown": return "Steady Guidance for Your Next Chapter";
        case "military": return "Clear, Practical Downsizing Support";
        case "rural": return `How ${FEATURED_BROKER.firstName} Helps You Downsize`;
        default: return "How Real Property Planning Helps With Downsizing";
      }
    case "executor-support":
      switch (t) {
        case "premium": return `How ${FEATURED_BROKER.firstName} Supports Executors`;
        case "urban": return "Experienced Executor Real Estate Support";
        case "waterfront": return `How ${FEATURED_BROKER.firstName} Helps Executors Navigate the Process`;
        ...
        case "rural": return `How ${FEATURED_BROKER.firstName} Helps With Executor Duties`;
        default: return "How Real Property Planning Supports Executors";
      }
    case "attorney-fiduciary-support":
      switch (t) {
        case "premium": return `How ${FEATURED_BROKER.firstName} Works With Legal Professionals`;
        case "urban": return "A Reliable Resource for Attorneys and Fiduciaries";
        case "waterfront": return `How ${FEATURED_BROKER.firstName} Supports Attorneys and Fiduciaries`;
        case "smalltown": return "Working With Your Legal Team";
        case "military": return "Practical Support for Legal Professionals";
        case "rural": return `How ${FEATURED_BROKER.firstName} Partners With Attorneys`;
        default: return "How Real Property Planning Works With Legal Professionals";
      }
    case "valuation-guidance":
      switch (t) {
        case "premium": return `How ${FEATURED_BROKER.firstName} Provides Valuation Clarity`;
        case "urban": return "Experienced Valuation Support";
        case "waterfront": return `How ${FEATURED_BROKER.firstName} Brings Valuation Insight`;
        case "smalltown": return "Honest, Grounded Valuation Guidance";
        case "military": return "Clear Property Valuation Support";
        case "rural": return `How ${FEATURED_BROKER.firstName} Provides Honest Valuation`;
        default: return "How Real Property Planning Delivers Valuation Insight";
      }
    case "preparing-home-for-sale":
      switch (t) {
        case "premium": return `How ${FEATURED_BROKER.firstName} Manages Property Preparation`;
        case "urban": return "Coordinated Home Preparation Support";
        case "waterfront": return `How ${FEATURED_BROKER.firstName} Helps Prepare Your Home`;
        case "smalltown": return "Getting Your Home Ready, Practically";
        case "military": return "Efficient Home Preparation Guidance";
        case "rural": return `How ${FEATURED_BROKER.firstName} Coordinates Property Preparation`;
        default: return "How Real Property Planning Prepares Homes for Sale";
      }
    case "divorce-related-home-sales":
      switch (t) {
        case "premium": return `How ${FEATURED_BROKER.firstName} Handles Divorce-Related Sales`;
        case "urban": return "Objective Support for Divorce Property Sales";
        case "waterfront": return `How ${FEATURED_BROKER.firstName} Helps With Divorce-Related Property`;
        case "smalltown": return "Discreet, Professional Sale Guidance";
        case "military": return "Clear Guidance for Divorce-Related Sales";
        case "rural": return `How ${FEATURED_BROKER.firstName} Manages Divorce Property Sales`;
        default: return "How Real Property Planning Supports Divorce Sales";
      }
    case "trust-estate-property-sales":
      switch (t) {
        case "premium": return `How ${FEATURED_BROKER.firstName} Supports Trust Property Sales`;
        case "urban": return "Experienced Trust Property Sale Guidance";
        case "waterfront": return `How ${FEATURED_BROKER.firstName} Helps With Trust-Owned Property`;
        ...
        case "rural": return `How ${FEATURED_BROKER.firstName} Helps Sell Trust-Owned Property`;
...
export function getToneHowWeHelpLead(cityName: string, serviceName: string, tone?: CommunityTone): string {
  switch (tone) {
    case "premium":
      return `${FEATURED_BROKER.name}'s dual credentials as a licensed broker and certified appraiser inform every step of the ${serviceName.toLowerCase()} process in ${cityName}:`;
    case "urban":
      return `${FEATURED_BROKER.name} provides experienced guidance tailored to ${cityName}'s market:`;
    case "waterfront":
      return `${FEATURED_BROKER.name} brings locally informed guidance to ${serviceName.toLowerCase()} in ${cityName}:`;
    case "smalltown":
    case "rural":
      return `${FEATURED_BROKER.name} provides steady, hands-on guidance through the ${serviceName.toLowerCase()} process:`;
    case "military":
      return `${FEATURED_BROKER.name} provides clear, practical guidance for ${serviceName.toLowerCase()} in ${cityName}:`;
    default:
      return `${FEATURED_BROKER.name} provides practical, evidence-based guidance for every ${serviceName.toLowerCase()} engagement:`;
  }
}
...
export function getToneCTAHeading(cityName: string, serviceName: string, tone?: CommunityTone): string {
  switch (tone) {
    ...
    case "waterfront":
      return `Talk with ${FEATURED_BROKER.name} About Your Property`;
    default:
...
export function getToneCTABody(cityName: string, tone?: CommunityTone): string {
  switch (tone) {
    ...
    case "smalltown":
    case "rural":
      return `Contact ${FEATURED_BROKER.pronoun.object} to discuss your situation — no pressure, no obligation.`;
    default:
...
const deepIntroBank: Record<string, Record<CommunityTone, string>> = {
  "probate-estate-sales": {
    premium: `In established, higher-value communities, probate and estate sales carry added complexity — from navigating multiple interested parties to pricing homes that reflect both premium location and real-world condition. In {city}, {county}, ${FEATURED_BROKER.name} provides the practical, evidence-based guidance that executors, attorneys, and families need to handle estate property with clarity and confidence.`,
    urban: `Urban estate sales move quickly and involve diverse property types — condos, townhomes, multifamily, and single-family homes across distinct neighborhoods. In {city}, the pace and complexity of the market make experienced guidance essential. ${FEATURED_BROKER.name} works with executors and attorneys who need a broker who understands city-market dynamics and can price estate property based on actual condition and realistic demand.`,
    suburban: `In family-oriented communities like {city}, estate and probate properties are often longtime family homes — well-loved but showing their age. Selling them effectively means understanding what buyers in {city} expect and how to position a home that may not match the newer construction nearby. ${FEATURED_BROKER.name} provides practical guidance grounded in local market knowledge and condition-based pricing.`,
    waterfront: `Waterfront and water-influenced communities present distinct challenges for estate sales — from view-driven premiums to properties with aging bulkheads, marine exposure, or unique lot configurations. In {city}, ${FEATURED_BROKER.name} helps executors and families price and prepare estate property with the local insight that waterfront markets require.`,
    smalltown: `In smaller communities like {city}, estate sales are personal — the family is often well-known, and the property may have been in the family for decades. ${FEATURED_BROKER.name} brings respectful, experienced guidance to probate and estate property sales in {city}, providing honest pricing and clear coordination without the pressure of a high-volume brokerage.`,
    military: `Military-connected communities like {city} experience frequent relocations, PCS-related sales, and estate transitions that may involve service members or their families. ${FEATURED_BROKER.name} provides steady, practical guidance for probate and estate sales in {city}, working within the timelines and constraints that military families often face.`,
    rural: `Rural and semi-rural estate properties in {city} often include features that standard market tools overlook — acreage, outbuildings, well and septic systems, or mixed-use potential. ${FEATURED_BROKER.name} provides condition-based pricing and hands-on coordination for estate sales in {city} where property characteristics require experienced, on-the-ground evaluation.`,
  },
  "senior-transitions": {
    premium: `Senior transitions in established communities like {city} often involve homes with significant value and decades of memories. The sale needs to be handled with care — for the family and for the asset. ${FEATURED_BROKER.name} guides {city} families through the process with experienced pricing, vendor coordination, and a pace that respects both the market opportunity and the human reality of the move.`,
    urban: `In {city}'s urban environment, senior transitions can involve everything from condos in walkable neighborhoods to mid-century homes in established residential areas. The range of options — and the pace of the market — make experienced guidance essential. ${FEATURED_BROKER.name} helps families coordinate the sale of a longtime home alongside the logistics of a move to assisted or independent living.`,
    suburban: `Many families in {city} reach out when a parent is ready to transition from the family home — a home they may have lived in for 20 or 30 years. ${FEATURED_BROKER.name} helps suburban families in {city} navigate the sale timeline, property preparation, and pricing decisions that come with this important life change.`,
    waterfront: `Senior transitions from waterfront or water-view homes in {city} carry unique considerations — from the premium that water access commands to the specialized maintenance these properties require. ${FEATURED_BROKER.name} helps families approach the sale with honest pricing and thoughtful coordination.`,
    smalltown: `In a close-knit community like {city}, a senior's decision to leave the family home is both practical and personal. ${FEATURED_BROKER.name} provides patient, clear guidance — helping families understand the property's realistic value, coordinate preparation, and manage the sale at a pace that works.`,
    military: `In {city}, senior transitions often intersect with military retirement or the needs of veteran families. ${FEATURED_BROKER.name} provides practical, respectful guidance for families coordinating a move from a longtime home to a more manageable living situation.`,
    rural: `Rural properties in {city} often present unique preparation challenges during a senior transition — deferred maintenance on larger lots, outbuildings, or systems that need evaluation. ${FEATURED_BROKER.name} provides honest, condition-based guidance for families managing these transitions.`,
  },
  "downsizing-services": {
    premium: `Downsizing from a premium home in {city} is a significant financial decision — the home may have appreciated substantially, and getting the pricing right matters. ${FEATURED_BROKER.name} helps {city} homeowners approach downsizing with a realistic understanding of their home's current value, the preparation that makes sense, and the timing that aligns with their next chapter.`,
    urban: `In {city}'s urban market, downsizing often means moving from a single-family home to a condo, townhome, or smaller residence in a different neighborhood. ${FEATURED_BROKER.name} helps homeowners in {city} navigate the transition with clear pricing guidance and practical coordination.`,
    suburban: `For homeowners in {city} whose children have moved out and who no longer need the space, downsizing is a practical decision that deserves experienced guidance. ${FEATURED_BROKER.name} works with families in {city} to assess their home's condition, understand realistic market value, and plan a sale that aligns with their next living arrangement.`,
    waterfront: `Downsizing from a waterfront property in {city} involves specific market considerations — waterfront premiums, specialized property features, and a buyer pool with distinct expectations. ${FEATURED_BROKER.name} provides practical, evidence-based guidance for homeowners ready to simplify.`,
    smalltown: `In {city}, downsizing often means leaving a home where the family has deep roots. ${FEATURED_BROKER.name} provides honest, practical guidance for homeowners who are ready to simplify — helping them understand what their property is worth and how to prepare it for sale.`,
    military: `Military families in {city} who are downsizing after retirement or a change in duty status benefit from a broker who understands the practical realities of military transitions. ${FEATURED_BROKER.name} provides clear, efficient guidance for the sale process.`,
    rural: `Downsizing from a rural property in {city} often involves considerations that standard downsizing guides don't address — acreage, outbuildings, septic systems, and rural buyer expectations. ${FEATURED_BROKER.name} provides on-the-ground assessment and honest pricing guidance.`,
  },
  "executor-support": {
    premium: `Executors managing estate property in {city}'s premium market face significant responsibility — the property may be high-value, the beneficiaries may have strong opinions, and the stakes of mispricing are real. ${FEATURED_BROKER.name} provides the valuation expertise and steady communication that executors in {city} need to fulfill their duties with confidence.`,
    urban: `Serving as executor for a property in {city}'s urban market can be overwhelming — especially for out-of-area executors who need local coordination. ${FEATURED_BROKER.name} provides experienced guidance for executors in {city}, managing the property assessment, preparation, and sale process from start to finish.`,
    suburban: `In family-oriented communities like {city}, executors often inherit a home that was the center of family life for decades. ${FEATURED_BROKER.name} helps executors in {city} navigate property assessment, preparation decisions, and the sale process with practical guidance and clear communication.`,
    waterfront: `Managing estate property in a waterfront community like {city} adds specific complexity — waterfront valuations, specialized property features, and a buyer market with particular expectations. ${FEATURED_BROKER.name} provides the locally informed guidance executors need.`,
    smalltown: `In {city}, executors often manage property in a community where the family is known and the situation is personal. ${FEATURED_BROKER.name} provides respectful, experienced support — handling property assessment, preparation, and sale coordination with the care the situation requires.`,
    military: `Executors in military-connected communities like {city} may be managing estate property while also navigating service-related obligations. ${FEATURED_BROKER.name} provides efficient, clear guidance that works within the executor's timeline and circumstances.`,
    rural: `Estate properties in rural areas like {city} often include features that complicate the executor's job — acreage, outbuildings, deferred maintenance, or systems that need inspection. ${FEATURED_BROKER.name} helps executors assess, prepare, and sell rural estate property with locally grounded expertise.`,
  },
  "attorney-fiduciary-support": {
    premium: `Attorneys and fiduciaries working with property in {city}'s premium market need a real estate partner who understands both the financial stakes and the professional standards involved. ${FEATURED_BROKER.name} works with legal professionals in {city} to provide defensible pricing, clear documentation, and responsive communication throughout the engagement.`,
    urban: `In {city}'s diverse urban market, attorneys and fiduciaries need a broker who can handle a wide range of property types and neighborhood contexts. ${FEATURED_BROKER.name} provides experienced support for legal professionals managing estate, trust, and guardianship property matters across {city}.`,
    suburban: `Legal professionals referring clients with property in {city} need a broker who communicates clearly, prices accurately, and follows through. ${FEATURED_BROKER.name} works with attorneys and fiduciaries in {city} as a reliable partner for estate, trust, and transition-related property matters.`,
    waterfront: `Waterfront property valuations in {city} require more than standard market comparisons. ${FEATURED_BROKER.name} provides attorneys and fiduciaries with condition-based assessments that account for the specific factors that drive value in waterfront and water-influenced markets.`,
    smalltown: `In a community like {city}, the relationship between legal professionals and their real estate partners matters. ${FEATURED_BROKER.name} provides attorneys and fiduciaries with dependable, locally informed guidance — the kind of support that earns ongoing referrals.`,
    military: `Attorneys serving military families in {city} need a real estate resource who understands PCS timelines, VA considerations, and the practical realities of service-connected property transitions. ${FEATURED_BROKER.name} provides clear, professional support for these situations.`,
    rural: `Rural property matters in {city} often involve unique characteristics — acreage, easements, water rights, or mixed-use zoning. ${FEATURED_BROKER.name} provides attorneys and fiduciaries with the property-specific analysis that rural markets require.`,
  },
  "valuation-guidance": {
    premium: `In {city}'s premium market, accurate valuation is the foundation of sound property decisions. Whether for estate settlement, trust administration, or pre-listing strategy, ${FEATURED_BROKER.name} provides valuation insight grounded in ${FEATURED_BROKER.name}'s dual credentials as a licensed broker and state-certified appraiser.`,
    urban: `Property values in {city} can vary dramatically from block to block. ${FEATURED_BROKER.name} provides condition-based valuation guidance that reflects {city}'s neighborhood-level market dynamics — not citywide averages or automated estimates.`,
    suburban: `For families and professionals in {city} who need a clear understanding of what a property is worth, ${FEATURED_BROKER.name} provides valuation guidance based on actual condition, comparable sales, and realistic market positioning — not automated tools that miss the details that matter.`,
    waterfront: `Waterfront and water-view properties in {city} are notoriously difficult to value with automated tools. ${FEATURED_BROKER.name} provides hands-on valuation guidance that accounts for view quality, waterfront access, marine exposure, and the specific buyer expectations of {city}'s market.`,
    smalltown: `In {city}'s smaller market, comparable sales may be limited and property characteristics can vary widely. ${FEATURED_BROKER.name} provides honest, condition-based valuation guidance that reflects the realities of {city}'s specific market — not a one-size-fits-all estimate.`,
    military: `Property valuations in military-connected communities like {city} are influenced by base proximity, PCS cycles, and VA-related buyer activity. ${FEATURED_BROKER.name} accounts for these local dynamics to provide accurate, defensible valuation guidance.`,
    rural: `Rural properties in {city} often have features — acreage, outbuildings, well/septic systems, or mixed-use potential — that automated valuation tools cannot evaluate. ${FEATURED_BROKER.name} provides in-person, condition-based assessment for properties that require local expertise.`,
  },
  "preparing-home-for-sale": {
    premium: `In {city}'s premium market, property presentation can significantly affect final sale price. ${FEATURED_BROKER.name} helps homeowners, executors, and families determine which preparations are worth the investment — balancing cost against realistic return in {city}'s competitive buyer environment.`,
    urban: `Preparing an estate or transition property for sale in {city}'s urban market often involves managing cleanout, vendor coordination, and presentation strategy across different property types. ${FEATURED_BROKER.name} handles the logistics so sellers can focus on the bigger picture.`,
    suburban: `Many homes in {city} need practical preparation before listing — cleanout of decades of belongings, minor repairs, or cosmetic updates. ${FEATURED_BROKER.name} helps families and executors in {city} make smart preparation decisions based on which improvements actually affect the sale outcome.`,
    waterfront: `Waterfront properties in {city} may require specialized preparation — addressing marine exposure, moisture concerns, or presentation that highlights water views and access. ${FEATURED_BROKER.name} coordinates preparation strategy that accounts for {city}'s waterfront buyer expectations.`,
    smalltown: `In {city}, preparing a longtime family home for sale is often more than a logistics challenge — it's personal. ${FEATURED_BROKER.name} provides practical guidance on what preparation makes sense, coordinating vendors and managing the process with care and efficiency.`,
    military: `Preparing a home for sale during a military transition in {city} often needs to happen on a compressed timeline. ${FEATURED_BROKER.name} provides efficient preparation coordination — cleanout, repairs, and presentation strategy managed within the seller's schedule.`,
    rural: `Rural properties in {city} may need preparation that goes beyond standard residential concerns — outbuilding maintenance, land clearing, septic inspection, or well certification. ${FEATURED_BROKER.name} coordinates all aspects of rural property preparation.`,
  },
  "divorce-related-home-sales": {
    premium: `Divorce-related property sales in {city} often involve homes with significant value, making accurate pricing essential for a fair settlement. ${FEATURED_BROKER.name} provides objective, practical, evidence-based guidance with the discretion and professionalism that {city} clients and their attorneys expect.`,
    urban: `In {city}'s urban market, divorce-related sales can involve a range of property types and complex neighborhood dynamics. ${FEATURED_BROKER.name} provides neutral, experienced guidance — working with both parties and their attorneys to manage the sale process fairly and efficiently.`,
    suburban: `Divorce-related home sales in {city} benefit from a broker who can remain objective while maintaining clear communication with both parties. ${FEATURED_BROKER.name} provides condition-based pricing, neutral coordination, and a professional process that reduces friction.`,
    waterfront: `Selling a waterfront property in {city} during a divorce requires specialized valuation — waterfront premiums, view corridors, and marine-related condition factors all affect fair pricing. ${FEATURED_BROKER.name} provides the objective, defensible guidance this situation demands.`,
    smalltown: `In a community like {city}, divorce-related sales require particular discretion and sensitivity. ${FEATURED_BROKER.name} provides professional, neutral coordination — honest pricing, clear communication, and a process focused on fair outcomes for both parties.`,
    military: `Divorce-related property sales in military communities like {city} may involve deployment timelines, VA loan considerations, or service-related constraints. ${FEATURED_BROKER.name} provides practical, neutral guidance that accommodates these realities.`,
    rural: `Rural divorce-related sales in {city} often involve properties with unique characteristics — acreage, outbuildings, or features that complicate standard valuation. ${FEATURED_BROKER.name} provides objective, property-specific assessment for fair settlement outcomes.`,
  },
  "trust-estate-property-sales": {
    premium: `Trust property sales in {city}'s premium market require transparency, defensible pricing, and clear communication with beneficiaries. ${FEATURED_BROKER.name} works with trustees and estate attorneys to manage trust-owned property sales in {city} with the professionalism and documentation that fiduciary standards demand.`,
    urban: `Managing trust property in {city}'s urban environment requires a broker who understands both the real estate market and the fiduciary obligations involved. ${FEATURED_BROKER.name} provides trustees with experienced guidance — from property assessment through closing.`,
    suburban: `For trustees managing property in {city}, the sale process needs to be transparent, well-documented, and defensible. ${FEATURED_BROKER.name} provides the clear communication and condition-based pricing that trust administration requires.`,
    waterfront: `Trust-owned waterfront properties in {city} require specialized valuation that accounts for premium features, limited comparable sales, and the specific documentation standards of trust administration. ${FEATURED_BROKER.name} provides the expertise trustees need.`,
    smalltown: `In {city}, trust property sales are often handled within a community context where reputation and integrity matter. ${FEATURED_BROKER.name} provides trustees with professional, well-documented sale coordination that meets fiduciary standards.`,
    ...
    rural: `Trust-owned rural properties in {city} may involve unique features — acreage, outbuildings, easements — that require specialized assessment. ${FEATURED_BROKER.name} provides trustees with the property-specific evaluation and clear documentation that fiduciary standards require.`,
  },
};
...
const deepWhyLocalBank: Record<string, Partial<Record<CommunityTone, string>>> = {
  "probate-estate-sales": {
    premium: `Probate properties in premium markets like {city} often appear to be worth more than their actual condition supports — or less, if unique features are overlooked. ${FEATURED_BROKER.name}'s on-site evaluation accounts for the specific factors that drive value in {city}: neighborhood positioning, lot characteristics, view quality, and the realistic cost of bringing deferred-maintenance properties to market standard. This is valuation guidance built on local knowledge, not automated assumptions.`,
    urban: `Estate properties in {city} exist within a complex urban market where neighborhood, zoning, and property type all affect value. A probate property in one part of {city} may be best positioned as a renovation opportunity; in another area, it may attract developers. ${FEATURED_BROKER.name}'s familiarity with {city}'s neighborhood-level dynamics allows him to advise executors and attorneys on the most realistic path to a successful sale.`,
    suburban: `In {city}'s suburban market, estate properties often compete against newer construction and recently updated homes. Buyers have expectations shaped by move-in-ready listings, which means a probate property with deferred maintenance or dated finishes needs careful pricing and preparation strategy. ${FEATURED_BROKER.name} helps families and executors understand how their specific property fits within {city}'s competitive landscape.`,
    waterfront: `Waterfront estate properties in {city} require specialized local knowledge — view premiums, waterfront access quality, marine exposure, and seasonal market patterns all affect how these properties should be priced and presented. Generic comparable sales from non-waterfront areas are unreliable guides. ${FEATURED_BROKER.name}'s appraisal credentials and local experience provide the waterfront-specific insight that estate sales in {city} require.`,
    smalltown: `In {city}'s smaller market, comparable sales may be limited and market timing can matter more than in larger communities. Estate properties here often have unique features — larger lots, older construction, or characteristics that don't fit neatly into automated valuation models. ${FEATURED_BROKER.name} provides the in-person, condition-based analysis that probate sales in {city}'s market require.`,
    military: `{city}'s real estate market is influenced by military presence — PCS cycles, VA-eligible buyer demographics, and proximity to installations all affect property demand and pricing. For estate sales in {city}, understanding these dynamics helps executors price accurately and reach the right buyer pool. ${FEATURED_BROKER.name}'s local knowledge accounts for the military-market factors that generic pricing tools miss.`,
    rural: `Rural estate properties in {city} often include features that standard market analysis doesn't cover — acreage, outbuildings, septic and well systems, or road access limitations. These characteristics can significantly affect value, and they require on-the-ground evaluation rather than desk-based estimates. ${FEATURED_BROKER.name} provides the hands-on assessment that rural probate and estate sales in {city} demand.`,
  },
  "senior-transitions": {
    premium: `Senior transition properties in {city} are often well-maintained but may show their age in ways that affect premium-market buyer expectations. Understanding how kitchen vintage, systems age, and cosmetic updates affect value in {city}'s competitive market helps families make smart preparation decisions. ${FEATURED_BROKER.name}'s appraisal background provides the condition-specific insight these decisions require.`,
    urban: `In {city}'s urban environment, a senior selling a longtime home may be leaving a neighborhood that has changed dramatically since they moved in. Understanding how urban redevelopment, shifting demographics, and neighborhood trajectory affect value helps families make informed decisions. ${FEATURED_BROKER.name}'s experience across {city}'s neighborhoods provides the local context these transitions require.`,
    suburban: `In {city}, longtime family homes being sold during a senior transition often have a character that newer homes lack — but they may also need updates to meet current buyer expectations. ${FEATURED_BROKER.name} helps families understand the balance: which improvements will affect the sale price, which won't, and how to prepare the home in a way that makes financial sense without unnecessary stress or expense.`,
    waterfront: `Waterfront homes in {city} that have been in the same family for years may have specialized maintenance needs — marine exposure wear, dock conditions, or drainage systems — that affect both value and buyer interest. ${FEATURED_BROKER.name} provides families with a realistic assessment of what these properties are worth and what preparation makes sense given {city}'s waterfront market dynamics.`,
    smalltown: `Senior transitions in {city} are often deeply personal — the family has lived in the same home for decades, and the community knows them. ${FEATURED_BROKER.name} brings local understanding and patience to these situations, helping families navigate the sale at a pace that works while providing honest guidance on what the home is worth in {city}'s specific market.`,
    military: `In {city}, many senior transitions involve veteran families whose property decisions intersect with retirement planning and military benefit considerations. ${FEATURED_BROKER.name}'s practical, locally grounded approach helps these families understand their property's realistic value and coordinate the sale alongside their broader transition planning.`,
    rural: `Rural senior transitions in {city} often involve properties with features that standard market analysis doesn't address — larger lots, outbuildings, deferred land maintenance, or systems like well and septic that need assessment. ${FEATURED_BROKER.name} provides the on-the-ground evaluation these properties need, helping families understand the full picture before making decisions.`,
  },
  "downsizing-services": {
    premium: `Homeowners downsizing from premium properties in {city} often face a tension between their home's emotional value and its market value. A home that has been well-loved for decades may not show as well as a recently updated property. ${FEATURED_BROKER.name}'s appraisal perspective helps owners see their home through the market's eyes — understanding exactly where their property stands and which preparation decisions make financial sense.`,
    urban: `Downsizing in {city}'s urban market involves specific considerations — the property's neighborhood trajectory, density changes, transit development, and competition from new construction all influence realistic pricing. ${FEATURED_BROKER.name}'s neighborhood-level market knowledge helps homeowners in {city} make clear-eyed decisions about timing, preparation, and pricing strategy.`,
    suburban: `For suburban homeowners in {city}, downsizing from a family home raises practical questions about value, preparation, and timing. The home may have been a perfect fit for a growing family but now competes against newer, more efficient construction. ${FEATURED_BROKER.name} helps {city} homeowners understand how condition, age, and neighborhood context affect what their home is actually worth.`,
    waterfront: `Downsizing from a waterfront property in {city} carries unique valuation considerations. The water premium is real, but it depends on specific factors — view quality, access type, shoreline condition — that vary property by property. ${FEATURED_BROKER.name}'s appraisal credentials provide the precise, waterfront-specific analysis that {city} homeowners need for confident pricing decisions.`,
    smalltown: `In {city}, homeowners who have lived in the same home for decades may have a strong sense of their property's value — but the market may see it differently. ${FEATURED_BROKER.name} provides honest, condition-based guidance that helps homeowners in {city}'s smaller market understand realistic pricing without the uncomfortable surprise that sometimes comes with listing a well-loved but dated home.`,
    military: `Military families downsizing in {city} often have practical, timeline-driven priorities. The sale needs to happen efficiently, pricing needs to be realistic, and the process needs to work within the constraints of a military transition. ${FEATURED_BROKER.name}'s practical approach to valuation and sale coordination aligns well with the no-nonsense expectations of military-connected homeowners.`,
    rural: `Rural downsizing in {city} often means selling a property with characteristics that standard pricing models can't evaluate — acreage, outbuildings, septic systems, or mixed-use potential. Real Property Planning's on-the-ground assessment accounts for the full scope of a rural property's features, providing the honest valuation guidance that {city} homeowners need for this important decision.`,
  },
  "executor-support": {
    premium: `Executors managing property in {city}'s premium market face heightened responsibility — the property's value is often significant, beneficiaries may have strong opinions, and mispricing can have real financial consequences. ${FEATURED_BROKER.name}'s dual credentials as a broker and certified appraiser provide executors with the defensible, property-specific pricing guidance that high-value estate situations require.`,
    urban: `Managing estate property in {city}'s urban market can be complex — the property's neighborhood context, zoning, and condition all affect strategy. An executor may need guidance on whether the property's best use is as a residence, a renovation opportunity, or a lot sale. ${FEATURED_BROKER.name}'s familiarity with {city}'s neighborhood dynamics helps executors make these decisions with confidence.`,
    suburban: `Executors in {city} often manage a home that was the center of family life — well-loved but possibly showing its age. Understanding how that property competes in {city}'s suburban market, where buyers often expect updated or move-in-ready homes, is critical for realistic pricing and smart preparation decisions. ${FEATURED_BROKER.name} provides this context through condition-based analysis, not guesswork.`,
    waterfront: `Waterfront estate properties in {city} present specific challenges for executors — view premiums, marine condition factors, seasonal market timing, and limited comparable sales all complicate pricing. ${FEATURED_BROKER.name}'s appraisal credentials and waterfront experience help executors navigate these factors with clarity and defensible pricing documentation.`,
    smalltown: `In {city}'s smaller market, executors benefit from working with someone who knows the community and understands the local dynamics. Comparable sales may be limited, and the property may have features that don't fit standard valuation models. ${FEATURED_BROKER.name} provides the hands-on, locally grounded guidance that executor responsibilities in {city} require.`,
    military: `Executors in {city}'s military-connected market need a broker who understands how base proximity, PCS cycles, and VA buyer demographics affect realistic property demand and pricing. ${FEATURED_BROKER.name} accounts for these military-market factors to help executors price estate property accurately and reach the right buyer pool efficiently.`,
    rural: `Rural estate properties in {city} often include features that complicate an executor's job — acreage, outbuildings, well/septic systems, easements, or mixed-use zoning. Standard desktop valuations miss these factors. ${FEATURED_BROKER.name} provides the on-the-ground evaluation that rural estate property in {city} requires for responsible pricing and sale coordination.`,
  },
  "attorney-fiduciary-support": {
    premium: `Attorneys and fiduciaries managing property matters in {city}'s premium market need a real estate partner who provides defensible pricing opinions, not aspirational estimates. ${FEATURED_BROKER.name}'s certified appraisal background produces documentation that withstands court scrutiny and beneficiary challenge — the kind of property-specific analysis that premium estate and trust situations demand.`,
    urban: `Legal professionals with property matters in {city}'s diverse urban market need a broker who can navigate different property types, neighborhood contexts, and market dynamics. ${FEATURED_BROKER.name}'s experience across {city}'s neighborhoods — combined with ${FEATURED_BROKER.name}'s appraisal credentials — provides the versatile, defensible real estate support that attorneys and fiduciaries need for their client matters.`,
    suburban: `In {city}'s suburban market, attorneys and fiduciaries need a broker who delivers consistent, reliable service — clear communication, defensible pricing, and professional follow-through on every engagement. Real Property Planning's practice is built on exactly this kind of dependability, which is why legal professionals in {city} refer clients with confidence.`,
    waterfront: `Waterfront property matters in {city} require specialized valuation knowledge that most brokers don't have. View premiums, shoreline access quality, marine condition factors, and seasonal demand patterns all affect defensible pricing. ${FEATURED_BROKER.name}'s appraisal background provides attorneys and fiduciaries with the waterfront-specific analysis these situations require.`,
    smalltown: `In {city}'s smaller market, the relationship between legal professionals and their real estate partners carries weight — both professionally and within the community. ${FEATURED_BROKER.name} provides the kind of dependable, locally informed service that earns trust over time, handling property matters with the care and discretion that {city}'s community context requires.`,
    military: `Attorneys serving military families in {city} need a real estate partner who understands PCS timelines, VA loan considerations, and the practical realities of military-connected property transitions. ${FEATURED_BROKER.name}'s familiarity with {city}'s military-influenced market dynamics allows him to provide the efficient, knowledgeable support these situations require.`,
    rural: `Rural property matters in {city} often involve characteristics that standard market analysis doesn't address — acreage valuations, easement questions, water rights, septic/well conditions, and access issues. ${FEATURED_BROKER.name} provides attorneys and fiduciaries with the hands-on, rural-specific property analysis that these matters require for defensible decision-making.`,
  },
  "valuation-guidance": {
    premium: `Accurate valuation in {city}'s premium market requires more than pulling comparable sales — it demands understanding how specific property characteristics interact with premium buyer expectations. ${FEATURED_BROKER.name}'s dual credentials as a broker and certified appraiser provide the granular, condition-based analysis that premium property valuation requires — whether for estate settlement, pre-listing strategy, or fiduciary reporting.`,
    urban: `Property values in {city} can shift dramatically across neighborhoods, blocks, and even individual properties based on lot potential, zoning, and use. ${FEATURED_BROKER.name}'s familiarity with {city}'s urban market allows him to provide valuation guidance that reflects real neighborhood-level dynamics rather than citywide averages or algorithm-generated estimates.`,
    suburban: `Suburban property valuation in {city} often comes down to condition — how a home compares to the recently updated homes that buyers are using as benchmarks. Automated tools frequently overstate value for properties with deferred maintenance or dated finishes. ${FEATURED_BROKER.name}'s appraisal-grounded assessment accounts for the gap between algorithm output and actual market reality.`,
    waterfront: `Waterfront valuations in {city} are among the most complex in residential real estate. View quality, shoreline type, access, marine exposure, and dock condition all affect value in ways that automated tools can't capture. ${FEATURED_BROKER.name}'s certified appraisal credentials provide the property-specific analysis that waterfront valuation in {city} demands.`,
    smalltown: `In {city}'s smaller market, valuation guidance needs to account for limited comparable sales and the wide variation in property characteristics. Standard automated tools trained on larger markets often misjudge properties in communities like {city}. ${FEATURED_BROKER.name} provides locally grounded, condition-based analysis that reflects what properties actually sell for in this specific market.`,
    military: `Property valuation in {city} is influenced by military-market factors that generic tools don't consider — base proximity, PCS-driven demand cycles, and VA buyer expectations all affect what properties are actually worth. ${FEATURED_BROKER.name} accounts for these dynamics to provide valuation guidance that is both accurate and defensible.`,
    rural: `Rural property valuation in {city} requires on-the-ground assessment — acreage, outbuildings, septic/well systems, road access, and land characteristics all contribute to value in ways that desktop analysis cannot capture. ${FEATURED_BROKER.name}'s hands-on approach provides the property-specific evaluation that rural valuation decisions in {city} require.`,
  },
  "preparing-home-for-sale": {
    premium: `In {city}'s premium market, the gap between an underprepared property and a properly presented one can represent a significant dollar difference. ${FEATURED_BROKER.name}'s appraisal-grounded perspective helps owners, executors, and trustees understand which preparations actually affect premium-market buyers' decisions — avoiding both over-investment in cosmetic upgrades and under-investment that leaves value on the table.`,
    urban: `Preparing estate and transition properties for sale in {city}'s urban market often involves navigating access logistics, vendor coordination in dense neighborhoods, and strategic decisions about property positioning. ${FEATURED_BROKER.name}'s experience with {city}'s urban market helps sellers focus preparation efforts where they'll have the most impact on the final outcome.`,
    suburban: `In {city}'s suburban market, preparation strategy is about understanding buyer expectations. Homes competing against recently updated listings need to present well — but not every update delivers return. ${FEATURED_BROKER.name} helps families and executors in {city} make practical, budget-conscious preparation decisions based on what actually affects sale price in their specific neighborhood.`,
    waterfront: `Waterfront property preparation in {city} extends beyond standard residential concerns — marine exposure wear, deck and dock condition, moisture management, and view presentation all matter to waterfront buyers. ${FEATURED_BROKER.name} helps sellers understand which waterfront-specific preparations are worth the investment given {city}'s buyer expectations and seasonal market patterns.`,
    smalltown: `Preparing a home for sale in {city} is often deeply personal — the home may have been in the family for decades. ${FEATURED_BROKER.name} helps families approach preparation practically: which improvements matter in {city}'s local market, which are not worth the investment, and how to manage the cleanout and vendor coordination with care and efficiency.`,
    military: `Preparing a home for sale in {city} on a military timeline requires efficiency and clear priorities. ${FEATURED_BROKER.name} helps military families and executors focus preparation on the improvements that actually affect sale outcomes — not cosmetic perfection, but practical preparation that positions the property well for {city}'s buyer pool within the available timeframe.`,
    rural: `Rural property preparation in {city} often extends well beyond the house — outbuildings, land maintenance, fencing, well/septic condition, and access paths all need assessment. ${FEATURED_BROKER.name} coordinates all aspects of rural preparation, helping sellers understand which investments matter and which are not worth the expense given {city}'s rural buyer expectations.`,
  },
  "divorce-related-home-sales": {
    premium: `Divorce-related sales in {city}'s premium market carry high financial stakes — accurate pricing is essential for a fair settlement, and both parties typically want defensible numbers. ${FEATURED_BROKER.name}'s certified appraisal background provides the objective, property-specific analysis that premium divorce sales require — pricing grounded in actual condition and market positioning, not automated estimates that miss critical factors.`,
    urban: `In {city}'s diverse urban market, divorce-related property valuations are complicated by neighborhood dynamics, property type variations, and rapidly shifting market conditions. ${FEATURED_BROKER.name}'s neighborhood-level expertise provides both parties and their attorneys with pricing guidance that reflects the property's specific location and condition — not citywide averages.`,
    suburban: `Divorce-related home sales in {city}'s suburban market require objective guidance from a broker who won't take sides. ${FEATURED_BROKER.name} provides condition-based pricing that both parties can rely on — grounded in local comparable sales and honest assessment of the property's condition, not emotional attachment or aspirational estimates.`,
    waterfront: `Waterfront divorce sales in {city} are inherently complex — the water premium is real but varies significantly by specific view, access, and condition. Both parties need defensible pricing that accurately reflects these factors. ${FEATURED_BROKER.name}'s appraisal credentials provide the objective, waterfront-specific analysis that fair settlement requires.`,
    smalltown: `Divorce-related property sales in {city} add a layer of community sensitivity. Both parties benefit from a broker who can handle the situation with discretion and professionalism while providing honest, defensible pricing in a market where comparable sales may be limited. ${FEATURED_BROKER.name} brings that combination of objectivity and local understanding.`,
    military: `Military divorce sales in {city} may involve unique considerations — BAH calculations, PCS timing, VA loan assumptions, and service-related scheduling constraints. ${FEATURED_BROKER.name}'s familiarity with {city}'s military-connected market dynamics helps both parties and their attorneys navigate these factors alongside the standard property sale process.`,
    rural: `Rural divorce sales in {city} often involve properties with features that make standard valuation complicated — acreage, outbuildings, water rights, or mixed-use potential. Real Property Planning's on-the-ground assessment captures these rural-specific factors, providing both parties with the defensible, comprehensive property analysis that fair settlement requires.`,
  },
  "trust-estate-property-sales": {
    premium: `Trust property sales in {city}'s premium market require pricing that meets fiduciary standards — defensible, well-documented, and grounded in actual property analysis. ${FEATURED_BROKER.name}'s dual credentials provide trustees with the professional-grade valuation and sale coordination that premium trust properties require, supporting both sound decision-making and clear beneficiary communication.`,
    urban: `Managing trust property in {city}'s urban market requires understanding how diverse property types, neighborhood dynamics, and market pace affect sale strategy. ${FEATURED_BROKER.name}'s experience across {city}'s neighborhoods helps trustees navigate these considerations while maintaining the documentation and communication standards that trust administration demands.`,
    suburban: `Trust property sales in {city}'s suburban market benefit from a broker who combines reliable execution with transparent pricing documentation. ${FEATURED_BROKER.name}'s approach — condition-based assessment, clear communication, and consistent follow-through — provides the professional partnership that trustees and their attorneys need for fiduciary-appropriate property disposition.`,
    waterfront: `Trust-owned waterfront properties in {city} present specialized valuation challenges — view premiums, marine condition factors, and seasonal market patterns all affect pricing in ways that standard comparisons don't capture. ${FEATURED_BROKER.name}'s appraisal credentials provide the property-specific analysis that waterfront trust property sales require for defensible, fiduciary-appropriate pricing.`,
    smalltown: `Trust property sales in {city}'s smaller market require a broker who combines local market knowledge with the documentation standards that fiduciary responsibilities demand. ${FEATURED_BROKER.name} provides that combination — locally grounded assessment, clear pricing rationale, and communication that keeps trustees, beneficiaries, and attorneys informed throughout the process.`,
    military: `Trust property in {city}'s military-connected market requires understanding how base proximity, PCS cycles, and VA buyer activity affect realistic pricing and market timing. ${FEATURED_BROKER.name} accounts for these local dynamics while maintaining the professional documentation and communication standards that trust administration requires.`,
    rural: `Rural trust properties in {city} often include characteristics — acreage, outbuildings, land features, access considerations — that standard market analysis doesn't address. ${FEATURED_BROKER.name} provides the hands-on evaluation and clear documentation that trustees need for defensible pricing and responsible sale coordination of rural trust-held property.`,
  },
};
...
const deepCTAHeadings: Record<CommunityTone, string[]> = {
  ...
  suburban: [
    ...
    `Talk With ${FEATURED_BROKER.name} About Your {city} Home`,
  ...
  smalltown: [
    ...
    `Let ${FEATURED_BROKER.name} Help With Your {city} Property`,
  ...
  rural: [
    ...
    `Let ${FEATURED_BROKER.name} Help With Your Property`,
  ],
};
...
const deepCTABodies: Record<CommunityTone, string[]> = {
  premium: [
    ...
    `${FEATURED_BROKER.name} provides experienced, discreet guidance for clients and professionals in {city}.`,
    "Reach out for a conversation about your property and what the path forward looks like.",
  ],
  urban: [
    `Contact ${FEATURED_BROKER.pronoun.object} to discuss your {city} property situation — no obligation, no pressure.`,
    "Schedule a consultation to get clear, practical guidance for your property decision.",
    `Reach out to discuss your situation and learn how ${FEATURED_BROKER.name} can help.`,
  ],
  suburban: [
    `${FEATURED_BROKER.name} provides clear, experienced guidance for families and professionals in {city}.`,
    "Schedule a consultation to discuss your property, your timeline, and your questions.",
    `Contact ${FEATURED_BROKER.pronoun.object} for an honest conversation about your {city} property and your options.`,
  ],
  waterfront: [
    "Reach out to discuss your {city} waterfront or water-view property and your options.",
    `${FEATURED_BROKER.name} provides locally informed guidance for property transitions in {city}.`,
    "Schedule a conversation about your property and what the process looks like from here.",
  ],
  smalltown: [
    `Contact ${FEATURED_BROKER.pronoun.object} to discuss your situation — no pressure, no obligation.`,
    "Reach out for an honest conversation about your property and your options.",
    `${FEATURED_BROKER.name} provides steady, experienced guidance for families in {city}.`,
  ],
  military: [
    "Schedule a consultation to discuss your property situation and timeline.",
    `${FEATURED_BROKER.name} provides clear, practical guidance that works within your schedule.`,
    `Contact ${FEATURED_BROKER.pronoun.object} to discuss your property — straightforward guidance, no pressure.`,
  ],
  rural: [
    `Contact ${FEATURED_BROKER.pronoun.object} to discuss your property — honest guidance based on actual property evaluation.`,
