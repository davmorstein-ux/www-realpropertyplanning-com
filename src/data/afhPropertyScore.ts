 ...
 *     required.
 *   WAC 388-76-10780 (DSHS inspection checklist): one flushing toilet per
 *     five persons, reachable without passing through another resident's room.
 ...
 *
 * NOT YET VERIFIED (David is checking): minimum clear width of the required
 * exit door, and the bedroom doorway width DSHS licensors expect. They appear
 * on the checklist as "measure and confirm", with no number stated.
 *
...
export type CategoryId = "bedrooms" | "stepFree" | "bathrooms" | "exits" | "site" | "systems";

export const CATEGORIES: { id: CategoryId; label: string; max: number }[] = [
  { id: "bedrooms", label: "Bedrooms and layout", max: 28 },
  { id: "stepFree", label: "Step-free living", max: 25 },
  { id: "bathrooms", label: "Bathrooms and care", max: 17 },
  { id: "exits", label: "Getting out safely", max: 15 },
  { id: "site", label: "The site", max: 10 },
  { id: "systems", label: "Major systems", max: 5 },
];
...
export const QUESTIONS: ScoreQuestion[] = [
  ...
  },
  {
    ...
    why: "State law protects adult family homes from zoning restrictions, but private covenants are a separate matter and should be read before anything else.",
    allowNotSure: true,
    options: [
      { id: "no", label: "No" },
      { id: "yes", label: "Yes" },
  ...
  },
  {
    id: "permits",
    section: "feasibility",
    prompt: "Are there additions, a garage conversion, or finished spaces that might not have permits?",
    why: "If you are counting on that space for a bedroom or bathroom, its legal status matters. Unpermitted work can be expensive to legalize.",
    allowNotSure: true,
    options: [
      { id: "no", label: "No" },
      { id: "yes", label: "Yes, or it looks like it" },
    ],
  },
  {
    id: "access",
  ...
  /* ---------- the score: bedrooms and layout, 28 ---------- */
  {
    ...
    why: "A resident bedroom needs 80 square feet of usable floor. The only cure for too few main-level bedrooms is an addition, the most expensive fix there is.",
    allowNotSure: true,
    options: [
      { id: "0", label: "None", points: 0 },
      { id: "1-2", label: "1 or 2", points: 6 },
      { id: "3", label: "3", points: 12 },
      { id: "4", label: "4", points: 17 },
      { id: "5", label: "5", points: 20, strength: "Five main-level bedrooms" },
      { id: "6+", label: "6 or more", points: 23, strength: "Six or more main-level bedrooms" },
    ],
  },
  {
    id: "convertible",
  ...
  /* ---------- step-free living, 25 ---------- */
  {
    ...
    why: "A ramp can be no steeper than 1 inch of rise per foot of length. Two steps need about 14 feet of ramp. Five steps need about 35 feet, plus landings, which may not fit the lot.",
    allowNotSure: true,
    options: [
      ...
      { id: "5+", label: "5 or more, or a steep approach", points: 0, cure: { cls: "C", project: "Ramp a tall entry, if the site allows it" } },
    ],
  },
  {
    id: "oneLevel",
    ...
    allowNotSure: true,
    options: [
      { id: "yes", label: "Yes", points: 10, strength: "True one-level living" },
      { id: "mostly", label: "Mostly", points: 5 },
  ...
  },
  {
    ...
    why: "Every interior level change is a fall risk and a barrier to a walker or wheelchair. Leveling a floor is a real construction project.",
    allowNotSure: true,
    options: [
      { id: "none", label: "None", points: 5 },
      { id: "one", label: "One spot", points: 3, cure: { cls: "B", project: "Level an interior step-down" } },
      { id: "several", label: "Several", points: 0, cure: { cls: "C", project: "Level several interior floor changes" } },
    ],
  },

  /* ---------- bathrooms and care, 17 ---------- */
  {
    id: "mainBaths",
    section: "score",
    category: "bathrooms",
    prompt: "How many full bathrooms are on the main level?",
    why: "DSHS expects at least one toilet for every five people in the home. Adding a bathroom is one of the more expensive conversions.",
    allowNotSure: true,
    options: [
      { id: "0", label: "None", points: 0, cure: { cls: "C", project: "Add main-level bathrooms" } },
      { id: "1", label: "1", points: 5, cure: { cls: "B", project: "Add a second main-level bathroom" } },
      { id: "2", label: "2", points: 10 },
      { id: "3+", label: "3 or more", points: 12, strength: "Three or more main-level bathrooms" },
    ],
  },
  {
    id: "bathReach",
    section: "score",
    category: "bathrooms",
    prompt: "Can every bedroom reach a bathroom without walking through another bedroom?",
    why: "A resident must be able to reach a toilet and bathing without passing through someone else's room.",
    allowNotSure: true,
    options: [
      { id: "yes", label: "Yes", points: 2 },
      { id: "no", label: "No", points: 0, cure: { cls: "B", project: "Rework the layout so each bedroom reaches a bathroom" } },
    ],
  },
  {
    id: "shower",
  ...
  /* ---------- getting out safely, 15 ---------- */
  {
    ...
    allowNotSure: true,
    options: [
      { id: "1", label: "1", points: 2, cure: { cls: "B", project: "Add an exterior exit door" } },
      { id: "2", label: "2", points: 6 },
  ...
  },
  {
    ...
    section: "score",
    category: "exits",
    prompt: "Does every main-level bedroom have a window that opens?",
    why: "Each resident bedroom needs an escape window. You will measure the sill height and the opening at the showing.",
    allowNotSure: true,
    options: [
      { id: "all", label: "Yes, all of them", points: 4 },
      { id: "some", label: "Some of them", points: 2, cure: { cls: "A", project: "Replace or enlarge a bedroom window" } },
  ...
  /* ---------- major systems, 5 ---------- */
  {
    ...
    section: "score",
    category: "systems",
    prompt: "Do you know of any major problem with the roof, foundation, electrical, or plumbing?",
    why: "Paint, flooring, and counters are a price question, not a suitability question, and are not scored. Major systems are different.",
    allowNotSure: true,
    options: [
      { id: "none", label: "None that I know of", points: 5 },
      { id: "yes", label: "Yes", points: 0, cure: { cls: "B", project: "Repair a major building system" } },
    ],
  },
  ...
  /* ---------- acquisition review (unscored, licensed homes only) ---------- */
  {
    ...
    why: "Every licensed home has a public record of inspections and enforcement. It is free and takes a few minutes.",
    allowNotSure: false,
    options: [
  ...
  },
  {
    ...
    why: "ECS and SBS contracts belong to the current owner and do not transfer. CBHS follows the resident, but a new owner must be enrolled and contracted to be paid.",
    allowNotSure: true,
    options: [
...
function feasibilityFlags(a: Answers): Flag[] {
  ...
  else f.push({ id: "hoa", level: "yellow", title: a.hoa === "yes" ? "Homeowners association" : "HOA status not confirmed", detail: "Read the covenants, conditions, and restrictions before spending money on inspections. State zoning protection does not settle what a private covenant says." });
  ...
  // Permits
  if (a.permits === "no") f.push({ id: "permits", level: "green", title: "No unpermitted work known", detail: "Still worth a quick check of county permit records." });
  else f.push({ id: "permits", level: "yellow", title: a.permits === "yes" ? "Possible unpermitted work" : "Permit history not confirmed", detail: "Compare the house to county permit records. Do not count a converted garage or addition as a bedroom or bathroom until its legal status is confirmed." });
  ...
  if (a.entrySteps === "5+" || a.entrySteps === "3-4") {
    ...
    f.push({
      ...
      title: tall ? "A tall entry may not have room for a ramp" : "Check that a ramp will fit",
      detail: `A ramp needs about one foot of length for every inch of rise, plus a 3 by 3 foot landing at the top, the bottom, and every turn, with handrails on both sides. ${tall ? "Five steps is roughly 35 feet of ramp." : "Three or four steps is roughly 21 to 28 feet of ramp."} Measure the rise and the available space at the showing.${a.residents !== "independent" ? " Residents who need help cannot use an exit path that includes steps." : ""}`,
    });
  }
  ...
  if (a.bathReach === "no") f.push({ id: "bathReach", level: "yellow", title: "A bedroom opens through another bedroom", detail: "A resident's route to a bathroom, and their route out of the home, cannot pass through another resident's bedroom." });
...
function showingChecklist(a: Answers): ChecklistItem[] {
  const c: ChecklistItem[] = [
    { title: "Measure each resident bedroom", text: "At least 80 square feet of usable floor for one resident, 120 for two. Closets and door swings do not count. Note any odd shape that would not fit a bed plus a walker.", source: "WAC 388-76-10690" },
    ...
    { title: "Measure doors and the hallway", text: "Clear width of the main exit door, each bedroom doorway, and the narrowest point of the hall. Write them down and confirm current minimums with the building official and the DSHS licensor." },
    { title: "Count the toilets", text: "Plan on one for every five people living in the home, each reachable without passing through a resident's bedroom.", source: "WAC 388-76-10780" },
  ...
  if (a.entrySteps && a.entrySteps !== "0") c.push({ title: "Measure the rise at each entry", text: "In inches, ground to threshold. A ramp needs about one foot per inch, plus a 3 by 3 foot landing at the top, bottom, and any turn. Pace off whether it fits.", source: "Building code R330.9" });
